...
def test_real_launch_diagnostic_is_baked_into_image_and_workflow_is_credential_free() -> None:
	...
	assert "for value in ('fixed', 'default', 'fixed')" in workflow
	assert "'--env', f'PROOF_FINGERPRINT_MODE={value}'" in workflow
	assert "'--env', 'PROOF_ENV_MODE=allowlist'" in workflow
